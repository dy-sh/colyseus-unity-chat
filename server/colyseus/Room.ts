...
export abstract class Room<T> extends EventEmitter {
  ...
  protected _previousStateEncoded: string;
  ...
  public send(client: Client, data: any): void {
    client.send(JSON.stringify([Protocol.ROOM_DATA, this.roomId, data]));
  }
  ...
  public broadcast(data: any): boolean {
    ...
    //   data = JSON.stringify([Protocol.ROOM_DATA, this.roomId, data]);
    // }

    var numClients = this.clients.length;
    while (numClients--) {
      this.clients[numClients].send(data);
    }
  ...
  private broadcastPatch(): boolean {
    ...
    let currentStateEncoded = JSON.stringify(currentState);
    ...
    let patches = jsonpatch.compare(this._previousState, currentState);
    // let patches = jsonpatch.compare(this._previousStateEncoded, currentStateEncoded);
    ...
    return this.broadcast(JSON.stringify([Protocol.ROOM_STATE_PATCH, this.roomId, p]));
  }
  ...
  private _onJoin(client: Client, options?: any): void {
    ...
    client.send(JSON.stringify([Protocol.JOIN_ROOM, this.roomId, this.roomName]));
  ...
  private _onLeave(client: Client, isDisconnect: boolean = false): void {
    ...
    if (!isDisconnect) {
      client.send(JSON.stringify([Protocol.LEAVE_ROOM, this.roomId]), { binary: true }, logError.bind(this));
    }
